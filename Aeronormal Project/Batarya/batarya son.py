...
# Batarya tüketimi optimizasyon fonksiyonu
def optimize_battery_usage(level, temperature):
    if level < 20:
        return "Düşük batarya seviyesi, enerji tasarrufu modu etkinleştirildi."
    if temperature > 60:
        return "Batarya sıcaklığı yüksek, soğutma sistemi etkinleştirildi."
    return ""
...
def simulate_battery():
    ...
    optimization_message.set(optimize_battery_usage(level, temperature))
...
optimization_message = tk.StringVar()
...
# Optimizasyon mesajı etiketi
optimization_message_label = ttk.Label(root, textvariable=optimization_message, style="Warning.TLabel")
optimization_message_label.grid(column=0, row=8, padx=10, pady=10)

# Şarj etme butonu
charge_button = ttk.Button(root, text="Şarj Et", command=toggle_charging)
charge_button.grid(column=0, row=9, padx=10, pady=10)

# Grafiklerin oluşturulması
fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(8, 6))
canvas = FigureCanvasTkAgg(fig, master=root)
canvas.get_tk_widget().grid(column=1, row=0, rowspan=10, padx=10, pady=10)

...
class BatteryManagementSystem:
    ...
    def optimize_settings(self, level, temperature):
        ...
        # Performans modu aktifken simülasyon hızını azalt
        if not self.performance_mode.get():
            self.root.after(10000, self.update_ui)  # Enerji tasarrufu modunda daha yavaş güncelleme
        else:
            self.root.after(5000, self.update_ui)  # Performans modunda daha hızlı güncelleme
    ...
    def toggle_charging(self):
        ...
        if self.charging.get():
            self.battery_warning.set("Charging in progress...")
            self.charge_button.config(bg='#f39c12')  # Şarj sırasında buton rengi değişir
        else:
            self.battery_warning.set("")
            self.charge_button.config(bg='#27ae60')  # Şarj durduğunda buton rengi geri döner
    ...
    def toggle_mode(self):
        ...
        if self.performance_mode.get():
            self.mode_button.config(bg='#2980b9')  # Performans modunda buton rengi mavi olur
        else:
            self.mode_button.config(bg='#e74c3c')  # Enerji tasarrufu modunda buton rengi kırmızı olur

...
class DroneSimulationApp:
    def __init__(self, root):
        ...
        # Drone resminin dosya yolunu belirleyin
        drone_image_path = os.path.join(os.path.dirname(__file__), "drone.png")
        if not os.path.exists(drone_image_path):
            messagebox.showerror("Hata", f"Drone resmi bulunamadı: {drone_image_path}")
            self.root.destroy()
            return

        self.drone_image = Image.open(drone_image_path)
        self.drone_image = self.drone_image.resize((self.cell_size, self.cell_size), Image.Resampling.LANCZOS)
        self.drone_photo = ImageTk.PhotoImage(self.drone_image)
    ...
    def draw_drone(self):
        self.drone = self.canvas.create_image(self.drone_pos[0]*self.cell_size, 
                                              self.drone_pos[1]*self.cell_size, 
                                              anchor=tk.NW, image=self.drone_photo)

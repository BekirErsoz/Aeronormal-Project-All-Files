...
from PyQt5.QtWidgets import QApplication, QMainWindow, QVBoxLayout, QWidget, QHBoxLayout, QLabel, QPushButton, QFileDialog, QInputDialog
...
class SensorThread(QThread):
    ...
    def parse_data(self, line):
        # Example parse function, assuming data comes as "theta,r,x,y,z"
        values = list(map(float, line.split(',')))
        return {'theta': values[0], 'r': values[1], 'x': values[2], 'y': values[3], 'z': values[4]}
...
class DroneApp(QMainWindow):
    def __init__(self):
        ...
        # Seri portu kullanıcıdan almak
        self.port, ok = QInputDialog.getText(self, "Seri Port Girişi", "Seri portu giriniz (örn. /dev/ttyUSB0):")
        if ok and self.port:
            self.sensor_thread = SensorThread(self.port)
            self.sensor_thread.update_signal.connect(self.update_sensor_data)
            self.sensor_thread.start()
        else:
            print("Seri port girişi yapılmadı veya iptal edildi.")
